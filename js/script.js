...
function open_fullscreen() {
    let game = document.getElementById("lessons-area");
    if (is_fullscreen) {
        // Exit fullscreen
        is_fullscreen = false;
        if (is_mobile_device()) {
            ...
            document.getElementById("lessons-player").style.display = "none";
        } else {
        ...
        is_fullscreen = true;
        if (is_mobile_device()) {
            document.getElementById("lessons-player").style.display = "block";
...
(function() {
    let last_offset = $("#new-lessons-section").children().length;
    let load_amount = 12;
    $('#load-more1').click((e) => {
        e.preventDefault();
    ...
    function append_content(data) {
        ...
        data.forEach((game) => {
            ...
            $("#new-lessons-section").append(html);
            $("#new-lessons-section .column:hidden").slice(0, load_amount).slideDown();
        });
    ...
    $('.b-action #favorite').on('click', function(e) {
        e.preventDefault();
        let data_id = $('.lessons-content').attr('data-id');
        let btn = $(this);
        $.ajax({
    ...
    let game_id;
    if ($('#comments').length) {
        game_id = $('.lessons-content').attr('data-id');
        $.ajax({
            url: '/includes/comment.php',
            type: 'POST',
            dataType: 'json',
